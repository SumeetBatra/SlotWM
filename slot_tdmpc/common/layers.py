...
from slot_tdmpc.models.slot_ae import SlotEncoder, SlotDecoder
	...
	for k in cfg.obs_shape.keys():
		if k == 'state':
			...
			if cfg.slot_wm:
				...
				out[k] = SlotEncoder(resolution=res, inp_channel=inp_channel, hid_dim=slot_dim)
			else:
	...
	for k in cfg.obs_shape.keys():
		if k == 'state':
			...
			if cfg.slot_wm:
				...
				out[k] = SlotDecoder(hid_dim=cfg.slot_dim, resolution=res, out_channel=out_channel)  # Or change resolution if needed
			else:
