...
class SlotEncoder(nn.Module):
    def __init__(self, resolution: Tuple[int, int], inp_channel: int, hid_dim: int, slot_attn: nn.Module):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        slots, keep_slots = self.slot_attention(x)
        return dict(slots=slots, keep_slots=keep_slots)
...
class SlotAutoEncoder(nn.Module):
    def __init__(self, obs_shape: Tuple[int, ...], num_slots: int, slot_dim: int, adaptive: bool,
    ...
    def batched_loss(self, batch):
        outs = self(batch['x'])
        bce_loss = F.binary_cross_entropy_with_logits(outs['x_hat_logits'], target=batch['x'], reduction='none').sum(
            (1, 2, 3)).mean()
        slot_reg = 0.0
        if self.adaptive:
