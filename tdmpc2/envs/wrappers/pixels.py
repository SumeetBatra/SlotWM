...
class PixelWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, cfg, env, num_envs, num_frames=3):
        ...
        if 'state' in self.base_env._init_raw_obs:
            ...
            self.state_stack = torch.zeros((*self.state_shape, self.num_frames), dtype=torch.float32,
    ...
    def observation(self, obs):
        ...
        rgb = self.rgb_stack.roll(shifts=-self._stack_idx, dims=-1).permute(0, 1, 2, 4, 3).reshape(
            (*self.rgb_shape[:-1], -1)).permute(0, 3, 1, 2)
        if self.include_state:
            state = self.state_stack.roll(shifts=-self._stack_idx, dims=-1).permute(0, 2, 1).reshape(
                (*self.state_shape[:-1], -1))
