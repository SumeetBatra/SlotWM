...
class TDMPC2:
    ...
    def plan(self, z, t0=False, eval_mode=False, task=None):
        ...
        if self.cfg.num_pi_trajs > 0:
            pi_actions = torch.empty(num_envs, self.cfg.horizon, self.cfg.num_pi_trajs, self.cfg.action_dim,
                                     device=self.device)
        ...
        for _ in range(self.cfg.iterations):
            ...
            actions[:, :, self.cfg.num_pi_trajs:] = (mean.unsqueeze(2) + std.unsqueeze(2) * \
                                                     torch.randn(num_envs, self.cfg.horizon,
            ...
            elite_actions = torch.gather(actions, 2,
                                         elite_idxs.unsqueeze(1).unsqueeze(3).expand(-1, self.cfg.horizon, -1,
                                                                                     self.cfg.action_dim))
            ...
            mean = torch.sum(score.unsqueeze(1) * elite_actions, dim=2) / (
                    score.sum(1, keepdim=True) + 1e-9)  # (num_envs, horizon, action_dim)
            std = torch.sqrt(torch.sum(score.unsqueeze(1) * (elite_actions - mean.unsqueeze(2)) ** 2, dim=2) / (
        ...
        actions = torch.zeros(num_envs, self.cfg.horizon, self.cfg.action_dim, dtype=actions.dtype,
