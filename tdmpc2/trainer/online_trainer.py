...
class OnlineTrainer(Trainer):
    ...
    def eval(self):
        ...
        for i in range(self.cfg.eval_episodes_per_env):
            ...
            done = torch.full((self.cfg.num_eval_envs,), False, device=(
                'cuda' if self.cfg.env_type == 'gpu' else 'cpu'))  # ms3: done is truncated since the ms3 ignore_terminations.
            ep_reward, t = torch.zeros((self.cfg.num_eval_envs,),
                                       device=('cuda' if self.cfg.env_type == 'gpu' else 'cpu')), 0
    ...
    def to_td(self, obs, num_envs, action=None, reward=None):
        ...
        if isinstance(obs, dict):
            ...
            obs = TensorDict(obs, batch_size=(),
                             device='cpu')  # before vectorization, obs must have its first dimension=1
        else:
        ...
        while self._step <= self.cfg.steps:
            ...
            if vec_done[0]:
                ...
                if self._step > 0:
                    tds = torch.cat(self._tds,
                                    dim=1)  # [num_envs, episode_len + 1, ..]. Note it's different from official vectorized code
                    ...
                    if seed_finish:
                        time_metrics.update(
                            ...
                            # self.cfg.num_envs * len(rollout_times)@steps_per_env@ /sum(rollout_times)
                    ...
                    assert len(
                        self._tds) == self.env.max_episode_steps + 1, f"{len(self._tds)} instead"  # ManiSkillVectorEnv wrapper required
            ...
            if self._step > self.cfg.seed_steps:
                action = self.agent.act(obs, t0=len(self._tds) == 1,
                                        eval_mode=False)  # t0 unchanged since all envs have same episode length
            else:
